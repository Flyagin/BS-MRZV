...
#include    "MnPrPrt.h"
long lActualPrTrObj;
#include    "../inc/hve_df.h"
#include    "../inc/hv_df.h"//"hv_df.h"
#include    "SmpWrp.h"
...
if (hldrHSU2LpduTRUnit.uchLpuHSU2State==0)
{
	...
	if(sLV.chTrHSU2)
	{
		...
		// AT91C_BASE_PIOD->PIO_SODR = (0x2);
		 lCtrCallsTrLpduHSU2++;
		ProcessTransmitLpduHSU2();//;ProcessTransmitStubSeqUart()
		}
		//else
...
;
	...
	{
		if(lBlockHSU2Ctr != 0)
		//Decrease Timer HSU2 Block
		lBlockHSU2Ctr--;
		else//
		{
			//Call Handler ReInit HSU2 State
			HSU2StallHundler();
		}
