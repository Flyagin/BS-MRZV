...
#define SAVE_CURSOR_POS		ESC"[s" 	// Save the cursor position for later restoration.  
#define RESTORE_CURSOR_POS 	ESC"[u"		//Restore the cursor position previously saved.  


//* Cursor Movement */
#define MOVEUP(num) 	ESC"["num"A" 		// Move the cursor up num positions  
#define MOVEDOWN(num)   ESC"["num"B" 		// Move the cursor down num positions  
#define MOVERIGHT(num)  ESC"["num"C" 		// Move the cursor right num positions  
#define MOVELEFT(num) 	ESC"["num"D"  	// Move the cursor left num positions  
#define MOVETO(row,col) ESC"["row";"col"H" 	// Move the cursor to the (col, row) position. Note that the row comes before column; that is, y comes before x. Either col or row can be omitted. Row and column both start with "1," not zero. (1, 1) corresponds to the top-left corner of the screen.  

//* Character Mode */

#define CHANGE_CHAR_MODE(attr)	ESC"["attr"m"	//Change the character mode with attribute attr. The attributes are numbers listed below.  
...
extern int AT91F_WaitKeyPressed(void);
